...
CREATE TABLE IF NOT EXISTS exam_mcq (
    ...
    created_by INT NOT NULL REFERENCES users(user_id),               -- Người tạo
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,                  -- Thời điểm tạo
    description TEXT                                                 -- Mô tả/ghi chú (nếu có)
);

-- Câu hỏi trắc nghiệm: loại câu, điểm, giải thích  ----- Các câu hỏi cx 1 đề thi sẽ cx 1 mã đề
CREATE TABLE IF NOT EXISTS mcq_questions (
    question_id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,    -- ID câu hỏi trắc nghiệm
    mcq_id INT NOT NULL REFERENCES exam_mcq(mcq_id) ON DELETE CASCADE, -- Thuộc đề thi nào
    question_text TEXT NOT NULL,                                     -- Nội dung câu hỏi
    question_type question_type_enum NOT NULL,                       -- Loại: Multiple choice/True-False
    score DECIMAL(4,2) DEFAULT 1.00,                                 -- Điểm câu hỏi
    explanation TEXT,                                                -- Giải thích/diễn giải đáp án
    usage_count INT DEFAULT 0,                                       -- Số lần đã đưa vào đề
    is_active BOOLEAN DEFAULT TRUE,                                  -- Còn sử dụng không
    image_url TEXT,                                                  -- Đường dẫn/URL hình ảnh câu hỏi
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP                   -- Thời điểm tạo
);
...
CREATE TABLE IF NOT EXISTS mcq_answers (
    ...
    image_url TEXT,                                                  -- Đường dẫn/URL hình ảnh đáp án
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP                   -- Thời điểm tạo
);
...
CREATE TABLE IF NOT EXISTS exam_schedule (
    ...
    time_end TIMESTAMP                                    -- Thời gian kết thúc ca thi
);
...
-- =====================================================
-- 14) SCHEDULE REGISTRATIONS (Quản lý thí sinh trong ca thi)
-- =====================================================
-- Bảng quản lý thí sinh trong ca thi (Many-to-Many)
CREATE TABLE IF NOT EXISTS schedule_registrations (
    id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_schedule INT NOT NULL REFERENCES exam_schedule(id_schedule) ON DELETE CASCADE,
    registration_id INT NOT NULL REFERENCES candidate_exam_registrations(registration_id) ON DELETE CASCADE,
    candidate_id INT NOT NULL REFERENCES candidates(candidate_id) ON DELETE CASCADE,
    joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,              -- Thời điểm tham gia ca thi
    is_allowed BOOLEAN DEFAULT TRUE,                            -- Admin có cho phép thi không
    added_by INT REFERENCES users(user_id),                     -- Admin thêm thí sinh (nếu là admin thêm)
    notes TEXT,                                                 -- Ghi chú (nếu có)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT unique_schedule_registration UNIQUE (id_schedule, registration_id)
);

-- Thêm index để truy vấn nhanh
CREATE INDEX IF NOT EXISTS idx_schedule_registrations_schedule ON schedule_registrations(id_schedule);
CREATE INDEX IF NOT EXISTS idx_schedule_registrations_registration ON schedule_registrations(registration_id);
CREATE INDEX IF NOT EXISTS idx_schedule_registrations_candidate ON schedule_registrations(candidate_id);


-- =====================================================
-- 15) VIEWS
-- =====================================================
-- View: Danh sách thí sinh trong ca thi
CREATE OR REPLACE VIEW v_schedule_candidates AS
SELECT
    sr.id,
    sr.id_schedule,
    es.schedule_code,
    es.schedule_password,
    es.time_start,
    es.time_limit,
    sr.registration_id,
    sr.candidate_id,
    ...
    em.exam_title,
    s.subject_name,
    -- Kiểm tra xem thí sinh đã làm bài chưa
    CASE WHEN ma.attempt_id IS NOT NULL THEN TRUE ELSE FALSE END as has_attempted
FROM schedule_registrations sr
JOIN exam_schedule es ON sr.id_schedule = es.id_schedule
JOIN exam_mcq em ON es.mcq_id = em.mcq_id
JOIN subjects s ON em.subject_id = s.subject_id
JOIN candidates c ON sr.candidate_id = c.candidate_id
JOIN users u ON c.user_id = u.user_id
LEFT JOIN mcq_attempts ma ON ma.id_schedule = es.id_schedule AND ma.candidate_id = c.candidate_id;


-- =====================================================
-- 16) FUNCTIONS HỖ TRỢ CA THI
-- =====================================================
-- Function: Thêm thí sinh vào ca thi (admin)
CREATE OR REPLACE FUNCTION add_candidate_to_schedule(
    p_schedule_id INT,
...
DECLARE
    ...
    v_result_id INT;
BEGIN
    -- Lấy candidate_id từ registration
    ...
    -- Thêm vào bảng schedule_registrations
    INSERT INTO schedule_registrations (id_schedule, registration_id, candidate_id, added_by, notes)
    VALUES (p_schedule_id, p_registration_id, v_candidate_id, p_added_by, p_notes)
    ON CONFLICT (id_schedule, registration_id) DO NOTHING
    RETURNING id INTO v_result_id;

    RETURN COALESCE(v_result_id, -1);
END;
$$ LANGUAGE plpgsql;

-- Function: Thí sinh tự tham gia ca thi bằng mã
CREATE OR REPLACE FUNCTION join_schedule_by_code(
    p_schedule_code TEXT,
    p_schedule_password TEXT,
    p_candidate_id INT
)
RETURNS TABLE(success BOOLEAN, message TEXT, schedule_id INT) AS $$
DECLARE
    v_schedule_id INT;
    v_mcq_id INT;
    v_subject_id INT;
...
BEGIN
    -- Tìm ca thi theo mã và mật khẩu
    SELECT es.id_schedule, es.mcq_id INTO v_schedule_id, v_mcq_id
    FROM exam_schedule es
    WHERE es.schedule_code = p_schedule_code
        AND (es.schedule_password IS NULL OR es.schedule_password = p_schedule_password);

    -- Kiểm tra ca thi có tồn tại không
    IF v_schedule_id IS NULL THEN
        RETURN QUERY SELECT FALSE, 'Mã ca thi hoặc mật khẩu không đúng'::TEXT, NULL::INT;
        RETURN;
    END IF;

    -- Lấy subject_id từ exam
    SELECT subject_id INTO v_subject_id
    FROM exam_mcq
    WHERE mcq_id = v_mcq_id;

    -- Kiểm tra thí sinh đã đăng ký môn thi chưa
    ...
    IF v_registration_id IS NULL THEN
        RETURN QUERY SELECT FALSE, 'Bạn chưa đăng ký hoặc chưa được duyệt cho môn thi này'::TEXT, NULL::INT;
        RETURN;
    END IF;

    -- Thêm vào ca thi
    INSERT INTO schedule_registrations (id_schedule, registration_id, candidate_id)
    VALUES (v_schedule_id, v_registration_id, p_candidate_id)
    ON CONFLICT (id_schedule, registration_id) DO NOTHING;

    RETURN QUERY SELECT TRUE, 'Tham gia ca thi thành công'::TEXT, v_schedule_id;
END;
$$ LANGUAGE plpgsql;


-- =====================================================
-- 17) TRIGGERS - TỰ ĐỘNG CẬP NHẬT UPDATED_AT
-- =====================================================
